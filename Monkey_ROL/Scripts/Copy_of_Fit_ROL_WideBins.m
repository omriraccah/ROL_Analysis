...
%baseline correct
data = (data-mean(mean_across_trials))/std_bl;
mdata = nanmean(data);
...
for b = 1:numbins
    ...
    if deactive == 0
        ...
        if bin_data{b,2} > ((std_bl*params.thr)+mean(mean_across_trials))
        ...
        % Check if bin mean exceeds threshold (1 = yes, 0 = no)
        if bin_data{b,2} < (mean(mean_across_trials)-(std_bl*params.thr))
...
for b = 1:numbins_bl
    ...
    if b == 1
                    ...
                    e_value = indsample(D, b_num+(times_nonover*params.bin_timewin));
                    bin_data_bl(b,1) = {mdata((s_value):((e_value)))};

    else

                    % Set data segment of interest and grab values
                    s_value = indsample(D, b_num+params.bin_timewin*(b-1));
                    e_value = indsample(D, b_num+(params.bin_timewin*(b-1))+(times_nonover*params.bin_timewin));
                    bin_data_bl(b,1) = {mdata(((s_value):((e_value))))};
