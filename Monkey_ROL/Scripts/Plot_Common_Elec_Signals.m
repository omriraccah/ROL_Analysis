function Plot_Common_Elec_Signals(fname,indchan,cond,ROL,timewin,save,prefix,colors)
...
for cn = 1:length(indchan)
    ...
    if cn == 1
        c_names = [c_names, strcat('PMC:', strcat(num2str(indchan(cn))))];

    end
    
    
    if cn == 2
        c_names = [c_names, strcat('LPC:', strcat(num2str(indchan(cn))))];
...
for i= 1:length(indchan)
    ...
    title('Common Electrode Plot ','fontsize',14, 'fontweight','bold')
...
if save
   if ~exist([pwd,filesep,'figs'],'dir')
        ...
        print('-opengl','-r300','-dpng',strcat([pwd,filesep,'figs',filesep,prefix,'Signal_ALL_conditions_', fname(end-11: end-4)]));
        delete(hfig)
end
...
for ii= 1:length(indchan)
    ...
    % First plot the signal to create the legend
        ...
        mcond = mean(D(indchan(ii),idt,tr_toplot),3);
        %mcond = abs(mcond)
...
if save
   if ~exist([pwd,filesep,'figs'],'dir')
        ...
        print('-opengl','-r300','-dpng',strcat([pwd,filesep,'figs',filesep,prefix,'Signal_ALLWithOnsets_conditions_', fname(end-11: end-4)]));
        delete(nfig)
end
...
%% Plot with onsets without line error bars && WITH TEXT ON PLOT
% -----------
...
%     CI= ([ME+nanmedian(ROL.onsets(:,ii)*1000) , nanmedian(ROL.onsets(:,ii)*1000)-ME]);
    ...
    ROL_str = num2str(nanmedian(ROL.onsets(:,ii)*1000));
    
    index = nanmedian(ROL.onsets(:,ii)*1000);
%     
%     hold on % Draw Text
%     text(nanmedian(ROL.onsets(:,ii)*1000),mcond(index),strcat('\leftarrow', ROL_str));
%     
    y = ylim;
%     
%     % Add a patch
...
if save
   if ~exist([pwd,filesep,'figs'],'dir')
        ...
        print('-opengl','-r300','-dpng',strcat([pwd,filesep,'figs',filesep,prefix,'Signal_ALLWithOnsets_&TEXT_conditions_', fname(end-11: end-4)]));
        delete(nfig)
end
