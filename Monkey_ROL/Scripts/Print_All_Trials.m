function Print_All_Trials(fname,indchan,conds,timewin, save,prefix,colors,labels)
...
for i=1:length(indchan)
    ...
    for sp = 1:length(labs) 
        ...
        for tr = 1:length(tr_toplot)
            ...
            %hold on
            % Add trial-by-trial error
            trial_Data = D(indchan(i),idt,tr_toplot(tr));
            plot(tim(idt), trial_Data,'Color', 'r','LineWidth',3)
            print('-opengl','-r300','-dpng',strcat([path,filesep,'figs',filesep,prefix,'Signal_conditions_',num2str(tr),'_',char(chanlabels(D,indchan(i)))]));
    ...
    hold on
    y = ylim;
    line([0 0],[-2 y(2)],'LineWidth',1,'Color','k'); %stim onset;
    ...
    title(['Channel ',char(chanlabels(D,indchan(i))), labels],'fontsize',14, 'fontweight','bold')
    ...
    if save
        ...
        print('-opengl','-r300','-dpng',strcat([path,filesep,'figs',filesep,prefix,'Signal_conditions_',char(chanlabels(D,indchan(i)))]));
        delete(hfig)
    end
