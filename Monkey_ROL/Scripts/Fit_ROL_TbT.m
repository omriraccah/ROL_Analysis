...
for b = 1:numbins
    ...
    if deactive == 0
        ...
        if b == 1
            ...
            % Set data segment of interest and grab values
            s_value = indsample(D, params.bin_timewin*(b-1)+0.001);
            e_value = indsample(D, (params.bin_timewin*(b-1)+0.001)+(times_nonover*params.bin_timewin));
            bin_data(b,1) = {mdata(((s_value-stim_onset_inx)+1):((e_value-stim_onset_inx)+1))};
...
for b = 1:numbins_bl
    ...
    if b == 1
            ...
            % Set data segment of interest and grab values
            s_value = indsample(D, b_num+params.bin_timewin*(b-1)+0.001);
            e_value = indsample(D, b_num+(params.bin_timewin*(b-1)+0.001)+(times_nonover*params.bin_timewin));
            bin_data_bl(b,1) = {mean_across_trials(((s_value-stim_onset_inx)+1):((e_value-stim_onset_inx)+1))};
