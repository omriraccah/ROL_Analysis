function Fig1A_Revised_ResponseType_Examples(fname,indchan,conds,timewin, save,~,colors,labels)

% Colors
% S1: 52,66,84
%Memory, Light grey: [169,169,169]/255
%Math Dark grey: [105,105,105]/255]
...
for i=1:length(indchan)
    ...
    % %stim onset
    line([0 0],[min(mcond)-20, max(mcond)+20],'LineWidth',1,'Color','k'); %stim onset;
...
%set(gca,'Xticklabel',[]) %to just get rid of the numbers but leave the ticks.
     ...
     xlim([-200 1200]);
     line(xlim,[0.0 0],'LineWidth',1,'Color','k');
...
set(gca,'YTick',[]);
 ...
 set(gca, 'FontName', 'Arial')
    ...
    % %stim onset
    ylim([-.3 1.6])
   % line([0 0],[min(mcond)-.5, max(mcond)+.5],'LineWidth',2,'Color','k'); %stim onset;
    ...
    if save
        ...
        print('-opengl','-r300','-dpng',strcat([path,filesep,'figs','S17_114_',char(chanlabels(D,indchan(i))),'_MMR']));
        %delete(hfig)
    end
