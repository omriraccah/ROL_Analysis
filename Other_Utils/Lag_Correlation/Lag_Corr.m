function [ATbT_Correlation_Values,lags] = Lag_Corr(fname,indchan,conds,timewin, save,suffix,colors,labels)
...
if length(labs) == 1 && length(indchan) == 2
    ...
    [MXCF,lags,bounds] = crosscorr(chan1_mcond,chan2_mcond,2000);
    bounds;
...
% check number of elecs
if length(labs) == 2 && length(indchan) == 2
    
    
    idt = tt(1):tt(2);

    tr_toplot1 = setdiff(indtrial(D,labs{1}),badtrials(D));
    tr_toplot2 = setdiff(indtrial(D,labs{2}),badtrials(D));

    chan1_mcond = squeeze(mean(D(indchan(1),idt,tr_toplot1),3));
    chan2_mcond = squeeze(mean(D(indchan(2),idt,tr_toplot2),3));
...
if length(labs) == 2 && length(indchan) == 1
...
chan1_cond = D(indchan(1),idt,tr_toplot1);
chan2_cond = D(indchan(1),idt,tr_toplot2);

chan1_mcond = squeeze(mean(D(indchan(1),idt,tr_toplot1),3));
chan2_mcond = squeeze(mean(D(indchan(1),idt,tr_toplot2),3));

% calculate across means

[ATbT_Correlation_Values,lags] = crosscorr(chan1_mcond,chan2_mcond,2000);
...
%     
% 
% %2ATbT_Correlation_Values = mean(TbT_Correlation_Values,1);
% 
%   
% %    
% % %  Plot the confidence bounds (horizontal lines) under the hypothesis that
